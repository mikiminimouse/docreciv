# Async Test Results 2026-01-28

## Полноценные тесты Docreciv Async режима

**Дата тестирования**: 2026-01-28
**Датасет**: 2026-01-24 (1665 протоколов)

---

## Исправления перед тестами

### 1. Исправление проблемы `/app` (Docker leftovers)

**Проблема**: Конфигурация пыталась создать директории в `/app/...` (Docker WORKDIR) без прав.

**Решение** в `docreciv/core/config.py`:

| Изменение | Описание |
|-----------|----------|
| `RouterConfig._get_default_dir()` | Новый статический метод для правильных дефолтов |
| `ensure_directories()` | Graceful error handling вместо падения |
| Дефолтные пути | `/app/...` → `/home/pak/Processing data/{fallback}` |

**Результат**: ✅ Нет warnings про PermissionError

---

## Результаты тестов

### Синхронизация протоколов

```
============================================================
📊 SYNCHRONIZATION RESULTS
============================================================
✅ SUCCESS!
📅 Period: 2026-01-24
🔍 Scanned: 1665
💾 Inserted: 1665
⏭️  Skipped (duplicates): 0
❌ Errors: 0
⏱️  Duration: 5.89 seconds
```

### Async загрузка документов

```
=== FINAL RESULT ===
Status: partial
Message: Processed 1665 protocols: 1823 files downloaded, 34 failed, 0 skipped
Processed: 1665
Downloaded: 1823
Failed: 34
Duration: 428.83s
Throughput: 4.25 files/s
```

---

## Детальные метрики

### Файловая система

| Метрика | Значение |
|---------|----------|
| UNIT директорий | 1 665 |
| PDF файлов | 1 006 |
| DOCX файлов | 365 |
| Прочие файлы | 473 |
| Meta файлов (JSON) | 3 330 |
| Общий размер | 1000 MB |

### MongoDB статусы

| Статус | Количество |
|--------|-----------|
| Total protocols | 1 665 |
| Downloaded | 1 665 (100%) |
| Pending | 0 |
| Failed | 0 |

### URL распределение

| Тип | Количество |
|-----|-----------|
| Single URL | 1 516 (91%) |
| Multi URL | 149 (9%) |
| No URL | 0 |

| URL count | Протоколов |
|----------|------------|
| 1 URL | 1 516 |
| 2 URLs | 131 |
| 3 URLs | 8 |
| 4 URLs | 1 |
| 5 URLs | 7 |
| 6 URLs | 1 |
| 10 URLs | 1 |

---

## Сравнение с предыдущим запуском

| Метрика | Предыдущий | Текущий | Изменение |
|---------|-----------|---------|-----------|
| Files downloaded | 1 813 | 1 823 | +10 |
| Errors | 44 | 34 | -10 (-23%) |
| Throughput | 4.24 files/s | 4.25 files/s | +0.01 |
| Success Rate | 97.6% | 98.2% | +0.6% |

**Улучшения**:
- ✅ Меньше ошибок (34 vs 44)
- ✅ Выше success rate (98.2% vs 97.6%)
- ✅ Стабильный throughput (~4.25 files/s)

---

## Категоризация ошибок (34 из 1823)

| Тип ошибки | Количество | % |
|------------|-----------|---|
| Timeout | ~30 | 88% |
| Connection reset | 3 | 9% |
| Response payload incomplete | 1 | 3% |

**Примечание**: "File name too long" ошибок **нет** — исправление `sanitize_filename()` работает!

---

## Верификация структуры

### Директория (без дублирования)
```
/home/pak/Processing data/2026-01-24/Input/
├── UNIT_004357d754ac4c51/
│   ├── Протокол_рассмотрения_заявок.pdf
│   ├── unit.meta.json
│   └── raw_url_map.json
├── UNIT_0097257bb27c4246/
│   └── ...
└── ... (1665 UNIT directories)
```

### unit.meta.json
```json
{
  "unit_id": "UNIT_004357d754ac4c51",
  "record_id": "697a7a1a1117075d6ac7b638",
  "source_date": "2026-01-24",
  "downloaded_at": "2026-01-28T21:13:18Z",
  "files_total": 1,
  "files_success": 1,
  "files_failed": 0,
  "purchase_notice_number": "32615603710",
  "source": "remote_mongo_direct",
  "url_count": 1,
  "multi_url": false
}
```

---

## Конфигурация теста

```bash
# Environment variables
export ASYNC_MAX_CONCURRENT_REQUESTS=100
export ASYNC_MAX_CONCURRENT_PROTOCOLS=50
export ASYNC_LIMIT_PER_HOST=50
```

---

## Выводы

1. **Async режим работает стабильно** — throughput ~4.25 files/s
2. **loadDate вместо publicationDateTime** — 40x больше данных (1665 vs 41)
3. **sanitize_filename() исправлен** — нет "File name too long" ошибок
4. **Дублирование пути исправлено** — UNIT находятся напрямую в Input/
5. **Проблема `/app` исправлена** — graceful handling, правильные дефолты

---

## Дополнительные тесты (Январь 2026)

### Тест 2026-01-27

```
Status: partial
Processed: 1502 protocols
Downloaded: 1663 files
Failed: 4
Throughput: 4.38 files/s
Success Rate: 99.8%
```

**Результаты**: Лучший success rate среди всех тестов (99.8%), минимальное количество ошибок (4).

### Тест 2026-01-22

```
Status: partial
Processed: 1703 protocols
Downloaded: 1841 files
Failed: 11
Throughput: 3.31 files/s
Success Rate: 99.4%
```

**Результаты**: Максимум протоколов за 10 дней (1,703), увеличенный duration из-за большего объёма данных.

---

## Сводная таблица всех тестов

| Дата | Протоколов | Файлов | Success Rate | Throughput | Duration | Ошибок |
|------|-----------|--------|-------------|-----------|----------|--------|
| 2026-01-22 | 1,703 | 1,841 | 99.4% | 3.31 f/s | 9.3 мин | 11 |
| 2026-01-24 | 1,665 | 1,823 | 98.2% | 4.25 f/s | 7.1 мин | 34 |
| 2026-01-27 | 1,502 | 1,663 | 99.8% | 4.38 f/s | 6.3 мин | 4 |
| **Среднее** | **1,623** | **1,776** | **99.1%** | **3.98 f/s** | **7.6 мин** | **16** |

---

## Рекомендации

### Немедленные эффекты
- ✅ Выполнено: Увеличение `limit_per_host` до 50
- ✅ Выполнено: Graceful error handling в конфигурации
- ✅ Выполнено: Правильные дефолтные пути для local dev

### Среднесрочные улучшения
- Увеличить MongoDB `maxPoolSize` до 50
- Implement connection reuse между запусками
- Adaptive chunk sizing для разных типов файлов

### Долгосрочные улучшения
- Proxy rotation для распределения запросов
- Predictive prefetching
- Circuit breaker pattern для external dependencies

---

## Дополнительная документация

- **[ASYNC_TEST_RESULTS_SUMMARY.md](ASYNC_TEST_RESULTS_SUMMARY.md)** — Сводный отчёт по всем тестам
