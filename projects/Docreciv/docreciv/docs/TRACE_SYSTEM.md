...
### Дубликаты registrationNumber

В исторических данных (до внедрения уникального индекса) могут присутствовать дубликаты `registrationNumber`. Это нормально — они возникли из-за повторных синхронизаций до создания уникального индекса.

**Статус**: Начиная с версии 2.1.0, уникальный индекс на `registrationNumber` предотвращает создание новых дубликатов.

Для очистки старых дубликатов можно использовать:

```python
# Удалить дубликаты, оставив самый старый документ
pipeline = [
    {"$group": {
...
for dup in collection.aggregate(pipeline):
    # Оставить первый документ, удалить остальные
